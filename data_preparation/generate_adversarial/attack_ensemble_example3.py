...
from attacker import AttackerMIFGSM
...
def main():
    ...
    output_dir = args.input_path + '_adv3'
    ...
    model3 = ResNet50(2, droprate=0)
    model_path3 = './weights/ResNet50.pth'
    ...
    model4 = VGG19_BN(2, droprate=0)
    model_path4 = './weights/VGG19_BN.pth'
    ...
    attackers=AttackerMIFGSM(eps=args.max_norm / 255.0, decay=1.0, steps=args.steps, low=1.0, high=1.25,
                              div_prob=args.div_prob, is_sigmoid=True,device=torch.device('cuda'))
    #attack
    for ind, (img,mask, label_true, filenames) in enumerate(loader):
        ...
        for bind, filename in enumerate(filenames):
            ...
            # print(ind, 'Attack on {}:'.format(os.path.split(filename)[-1]))
