...
def main():
    ...
    model = VGG16(args.num_class, droprate=args.droprate).cuda()
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % args.val_epochs == 0 and epoch>500:
            ...
            if epoch+1 != args.val_epochs+500:
                df_acc.to_csv('%s/report/VGG16.csv' % args.save_path, mode='a', index=None, header=None)
            else:
                df_acc.to_csv('%s/report/VGG16.csv' % args.save_path, mode='a', index=None)
            ...
            if best_loss > running_loss['BCE'] / len(test_loader):
                ...
                save_network(model, '%s/models/VGG16.pth' % args.save_path)
