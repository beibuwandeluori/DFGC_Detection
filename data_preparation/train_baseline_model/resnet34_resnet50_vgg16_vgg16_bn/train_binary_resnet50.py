...
def parse_args():
    ...
    parser.add_argument('--root_path', type=str, default='/home/chenhan/Celeb-DF-v2-face')           
    ...
    parser.add_argument('--adjust_lr_epochs', type=int, default=100)
...
def main():
    ...
    model = ResNet50(args.num_class, droprate=args.droprate).cuda()
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % args.val_epochs == 0 and epoch>200:
            model.train(False)
            model.eval()
            running_loss = {loss: 0 for loss in loss_name}
            ...
            if epoch+1 != args.val_epochs:
                df_acc.to_csv('%s/report/ResNet50.csv' % args.save_path, mode='a', index=None, header=None)
            else:
                df_acc.to_csv('%s/report/ResNet50.csv' % args.save_path, mode='a', index=None)
            ...
            if best_loss > running_loss['BCE'] / len(test_loader):
                ...
                save_network(model, '%s/models/ResNet50.pth' % args.save_path)
