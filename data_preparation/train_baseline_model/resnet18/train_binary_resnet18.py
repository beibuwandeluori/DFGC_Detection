...
def main():
    ...
    model = ResNet18(args.num_class, droprate=args.droprate).cuda()
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % args.val_epochs == 0 and epoch>200:
            ...
            df_acc['Acc'] = avg_acc
            if epoch+1 != (args.val_epochs + 200):
                df_acc.to_csv('%s/report/ResNet18.csv' % args.save_path, mode='a', index=None, header=None)
            else:
                df_acc.to_csv('%s/report/ResNet18.csv' % args.save_path, mode='a', index=None)
            ...
            if best_loss > running_loss['BCE'] / len(test_loader):
                ...
                save_network(model, '%s/models/ResNet18.pth' % args.save_path)
